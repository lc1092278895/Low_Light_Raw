...
import time
import argparse
from model_Modified import EnhanceNet
...
parser.add_argument('--gpu_mem', dest='gpu_mem', type=float, default=0, help="0 to 1, gpu memory usage")
...
parser.add_argument('--checkpoint_dir', dest='ckpt_dir', default='./checkpoint_Sony_0531_xiaorong_L1_SSIM', help='directory for checkpoints')
parser.add_argument('--sample_dir', dest='sample_dir', default='./sample_Sony_0531_xiaorong_L1_SSIM', help='directory for evaluating outputs')

parser.add_argument('--save_dir', dest='save_dir', default='./test_results_Sony_0531_xiaorong_L1_SSIM', help='directory for testing outputs')
parser.add_argument('--test_dir', dest='test_dir', default=r'G:/chenlong/Raw_Dataset/Sony/short', help='directory for testing inputs')# F:/chenlong/Dataset_Sony/Sony/short
parser.add_argument('--train_dir', dest='train_dir', default=r'G:/chenlong/Raw_Dataset/Sony/', help='directory for training inputs')# 路径后面有斜杆
...
if not os.path.exists("checkpoint_Sony_0531_xiaorong_L1_SSIM"):
    os.makedirs("checkpoint_Sony_0531_xiaorong_L1_SSIM")
if not os.path.exists("test_results_Sony_0531_xiaorong_L1_SSIM"):
    os.makedirs("test_results_Sony_0531_xiaorong_L1_SSIM")
if not os.path.exists("sample_Sony_0531_xiaorong_L1_SSIM"):
    os.makedirs("sample_Sony_0531_xiaorong_L1_SSIM")
...
def final_loss(S_low,high_im):
    ...
    loss = (alpha*pix_loss + beta*ssim_loss + theta*grad_loss )/10
    loss = (alpha * pix_loss + beta * ssim_loss) / 10
    # 消融实验1：仅使用L1
    # loss = pix_loss / 10
    return loss
...
def train():
    f = open('0531_xiaorong_L1_SSIM.txt', 'w')
    f.close()
    start_epoch = 0
    ...
    for epoch in range(start_epoch,args.epoch):
        ...
        if epoch % 50 ==0:
            f = open('0524_loss_xiaorong1.txt','a')
            if epoch > 1:
            ...
            torch.save(state,args.ckpt_dir+'/%4d_state_final.pth'%epoch)
    state = {'enhance':enhancenet.state_dict(),'enhance_optim':enhance_optim.state_dict(),'epoch':epoch}
    if epoch%10==0:
        torch.save(state,args.ckpt_dir+'/%4d_state_final.pth'%epoch)
    w_log.close()
...
def test():
    checkpoint = torch.load(args.ckpt_dir+'/3950_state_final.pth')
    enhancenet.load_state_dict(checkpoint['enhance'])
